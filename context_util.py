...
def save_frames(image, image_folder, index=None, image_name=None):
    if image is not None:
        ...
        if image_name:
            ...
            io.imsave(image_folder + str(index).zfill(5) + '.jpg', image)
...
def folder2vid(image_folder, output_dir, filename):
    ...
    video = cv2.VideoWriter(os.path.join(output_dir, filename), cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 24, (width, height))
    for image in images:
